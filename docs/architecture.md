...
- **Rendering** (`src/render/draw.mjs`): `drawFrame(ctx, state)` paints one
  ...
  - A blurred `MultiEffect` copy under the crisp frame adds a glow. It dates
    from the neon look. Whether the retro palette keeps it is open (below).
  - The renderer only reads state.
  - **Colors are one fixed retro palette**, `PALETTE` in `draw.mjs`
    (game-design §5). Nothing reads the omarchy theme. `Overlay.qml` also
    takes its window backdrop and fault-text color from `Draw.PALETTE`. This
    replaced the live-theme lookup through the `qs.Commons` `Color`
    singleton (commit `aeef455`), which the user rejected.
  - `src/render/draw.mjs` stays a plain ES module like `src/core` and never
...
1. Proving the live shell behaviour (loop stops after `hide`, frame pacing,
   which monitor the overlay opens on) means enabling the plugin in the
   user's running desktop.
2. A licence. The user's other plugins are MIT, but this repo has none until
   someone chooses.
3. Whether to keep the glow `MultiEffect` under the flat retro palette
   (game-design §5). It can only be judged by eye on the live overlay.
